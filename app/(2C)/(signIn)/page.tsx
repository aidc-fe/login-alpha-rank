...
export default function Home() {
  ...
  useEffect(() => {
    if (isSSO === undefined) {
      ...
      setCallbackUrl(
        `/api/oauth/authorize/default?redirect_uri=${redirect_uris?.[0]}&client_id=${client_id}&callbackUrl=${window.opener && window.name === "loginWindow" ? `${window.location.origin}/popup-login` : ""}${searchParams.get("utm_source") ? `&utm_source=${searchParams.get("utm_source")}&utm_type=sign_in` : ""}`
      );
    }
  }, [isSSO]);
